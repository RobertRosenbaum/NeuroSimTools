...
# Spiking neural net model
class SpikingModelISP(nn.Module):

    def __init__(self, recurrent, tausyn, readin=None, NeuronModel='EIF', NeuronParams={}, PlasticityParams={}):
        super(SpikingModelISP, self).__init__()
        ...
        if PlasticityParams==None:
            self.PlastcityOn = False
        else:
            self.PlastcityOn = True
            self.tauSTDP = PlasticityParams.get('tauSTDP',250)
            self.keep_sparsity = PlasticityParams.get('keep_sparsity',True)
            self.r0 = PlasticityParams.get('r0',0.005)
            self.eta_pre = PlasticityParams.get('eta_pre', 1)#PlasticityParams.get('eta_pre', nn.Parameter(torch.ones(self.N_recurrent)))
            self.eta_post = PlasticityParams.get('eta_post', 100.0)#PlasticityParams.get('eta_post', nn.Parameter(100.0*torch.ones(self.N_recurrent)))
    ...
    def forward(self, x0, dt, x=None, T=None, initial_V='random', initial_Y='zero', dtRecord = None, Tburn = 0, VIRecord = []):
        ...
        if self.PlastcityOn:
            e = torch.zeros(self.N_recurrent, requires_grad=this_req_grad).to(this_device)

            # Matrix of pairiwse plasticity coefficients.
            # This is the "Boss" matrix of pairwise learning rates.
            eta_Boss = torch.outer(self.eta_post, self.eta_pre)
            if self.keep_sparsity:
                eta_Boss[(torch.abs(self.recurrent_layer.weight) < 1e-9)] = 0.0
        else:
            e = None
        ...
        for i in range(Nt):
            ...
            spike_mask = (self.V>=self.Vth)
            self.V[spike_mask] = self.Vre
            S = spike_mask/dt
            #print(spike_mask.sum(axis=0))
            # Euler step for elig traces
            if self.PlastcityOn:
                e = e + dt*(-e+S)/self.tauSTDP
                #self.recurrent_layer.weight[spike_mask, :] -= torch.outer(self.eta_post[spike_mask], self.eta_pre*e) * Jmask[spike_mask, :]
                #self.recurrent_layer.weight[:, spike_mask] -= torch.outer(self.eta_post*(e-2*self.r0), self.eta_pre[spike_mask]) * Jmask[:, spike_mask]
                #print(spike_mask.shape,self.recurrent_layer.weight.shape)#,eta_Boss[spike_mask, :].shape,e.shape,(eta_Boss[spike_mask, :]*e).shape)
                for b in range(batch_size):
                    self.recurrent_layer.weight[spike_mask[b,:], :] -= eta_Boss[spike_mask[b,:], :]*e[b,:]/batch_size
                    self.recurrent_layer.weight[:, spike_mask[b,:]] -= ((((e[b,:]-2*self.r0))*eta_Boss[:, spike_mask[b,:]].T).T)/batch_size
